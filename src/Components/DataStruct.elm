...
type SponsorKind = Platine | Gold | Silver

computeCss:SponsorKind -> String
computeCss k =
        case k of
            Platine -> "footer-platinum-logo"
            Gold -> "footer-gold-logo"
            Silver -> "footer-silver-logo"
...
sponsorsItemDecoder = D.map3 Sponsors
...
globalData = D.map GlobalData
        (D.field "sponsors" sponsorsItemDecoder )

data : DataSource GlobalData
data = DataSource.File.jsonFile globalData  "_data/scalaIO_data.json"
