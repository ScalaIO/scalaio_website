...
sponsorToUrl k s = a[Attr.href s.url, Attr.target "_blank"][img [ src ("assets/sponsors/logo-" ++ s.name ++ ".svg"), alt s.name, class  ("flex " ++ (computeCss k)) ] []]

transform:List Sponsor -> SponsorKind  -> List (Html msg)
transform data k = List.map (sponsorToUrl k) data